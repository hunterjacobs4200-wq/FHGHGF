...
def parse_args():
    ...
    parser.add_argument("--output-yft", required=True)
...
def ensure_sollumz():
    # Headless Blender can run under a different user profile than your UI session.
    # Allow pointing directly to the Sollumz addon folder to make it deterministic.
    ...
    # Different installs may register as "sollumz" or "Sollumz"
...
def set_selected_sollum_type(type_name: str):
    ...
    if type_name not in valid:
        raise RuntimeError(f'Sollum type "{type_name}" is unavailable. Valid examples: {sorted(list(valid))[:8]}')
...
def main():
    args = parse_args()
    out_dir = os.path.dirname(os.path.abspath(args.output_yft))
    ...
    # Build minimal Sollumz vehicle hierarchy: DRAWABLE -> FRAGMENT.
    select_only(meshes)
    ...
    fragment = bpy.context.view_layer.objects.active

    select_only([fragment])
    ...
    if not copy_first_with_ext(out_dir, ".yft", args.output_yft):
        raise RuntimeError("No .yft produced. Ensure Sollumz native export provider is installed.")
